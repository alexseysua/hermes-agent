...
> Variant: automated ("trust me bro"). See /code for manual approval gate.
...
## Step 4.5: Goal Verification (opt-in)

After code review passes, offer goal verification:
- Use AskUserQuestion: "Run goal verification against the plan?" → Yes / No (Recommended)
- If Yes: Spawn `goal-verifier` agent with the plan directory
  - If `passed` → proceed to Step 5
  - If `gaps_found` → present gaps to user: "Implementation has gaps. Fix now or proceed?"
  - If `human_needed` → present items for human testing, ask for confirmation
