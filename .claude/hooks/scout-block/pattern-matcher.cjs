...
function matchPath(matcher, testPath) {
  ...
  // Skip paths outside the project — ignore library rejects '../' paths
  if (normalized.startsWith('..')) {
    return { blocked: false };
  }
...
/**
 * Load and merge patterns from multiple .ckignore files (global + project).
 * Falls back to DEFAULT_PATTERNS if no patterns found.
 *
 * @param {string[]} paths - Array of .ckignore file paths
 * @returns {string[]} Merged, deduplicated array of patterns
 */
function loadPatternsFromMultiple(paths) {
  const allPatterns = [];
  for (const p of paths) {
    if (p && fs.existsSync(p)) {
      try {
        ...
        console.error(`WARN: Failed to read ${p}:`, error.message);
      }
    }
  }
  const unique = [...new Set(allPatterns)];
  return unique.length > 0 ? unique : DEFAULT_PATTERNS;
}

module.exports = {
  loadPatterns,
  loadPatternsFromMultiple,
  createMatcher,
  matchPath,
  findMatchingPattern,
  DEFAULT_PATTERNS
