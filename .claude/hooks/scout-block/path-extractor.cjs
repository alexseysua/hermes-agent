 ...
 */
function normalizeExtractedPath(p) {
  if (!p) return '';
  ...
  // Convert absolute paths to relative (required by 'ignore' library)
  if (nodePath.isAbsolute(normalized)) {
    normalized = nodePath.relative(process.cwd(), normalized);
    // Re-normalize to forward slashes after relative() on Windows
    normalized = normalized.replace(/\\/g, '/');
