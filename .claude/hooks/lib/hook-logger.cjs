 ...
 * Auto-creates .logs/ directory and handles rotation (1000 lines max → keep last 500).
 * Uses only Node builtins (fs, path) — no external dependencies.
 * Every function is wrapped in try-catch — NEVER crashes.
 *
 * Exports: logHook(hookName, data), createHookTimer(hookName)
 */
...
/**
 * Rotate log file if it exceeds MAX_LINES, keeping the last TRUNCATE_TO lines.
 */
...
/**
 * Log a hook event as a JSONL entry.
 *
...
/**
 * Create a duration timer for a hook. Call .end(data) to log with elapsed ms.
 *
