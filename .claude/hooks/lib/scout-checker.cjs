 ...
 *
 * Provides unified interface to scout-block/* modules for reuse.
...
// Import scout-block modules (relative to lib/ → ../scout-block/)
...
// Build command allowlist - these are allowed even if they touch blocked paths
...
// python/python3 -m venv, py -m venv, uv venv, virtualenv
...
function stripCommandPrefix(command) {
  ...
  stripped = stripped.replace(/^(\w+=\S+\s+)+/, ''); // Strip env vars again (sudo env VAR=x cmd)
 ...
 * Does NOT split on newlines (heredoc bodies, multiline strings)
...
/**
 * Check if command should be allowed (build, venv, or tool command)
 ...
 * @param {string} [params.options.claudeDir] - Path to .claude directory
 * @param {boolean} [params.options.checkBroadPatterns=true] - Check overly broad glob patterns
 * @returns {{
 *   blocked: boolean,
 *   path?: string,
 *   pattern?: string,
...
function checkScoutBlock({ toolName, toolInput, options = {} }) {
  ...
  // Unwrap shell executor wrappers so inner command gets properly analyzed
  ...
  // Split compound commands (&&, ||, ;) and check each sub-command independently
  // Must split BEFORE isAllowedCommand because BUILD_COMMAND_PATTERN has no end anchor
  if (toolInput.command) {
  ...
  // Resolve .ckignore path and load patterns
