 ...
 *
 * Builds session context, rules, paths, and plan information for injection
 * into dev-rules-reminder.cjs and subagent-init.cjs hooks.
 *
...
/**
 * Resolve rules file path (local or global), with backward compat for workflows/
 * @param {string} filename - Rules filename
 * @param {string} [configDirName='.claude'] - Config directory name
 * @returns {string|null} Resolved display path or null
 */
...
const resolveWorkflowPath = resolveRulesPath;
 ...
 * @returns {{reportsPath: string, gitBranch: string|null, planLine: string, namePattern: string, validationMode: string, validationMin: number, validationMax: number}}
 */
...
// ═══════════════════════════════════════════════════════════════════════════
// USAGE / CONTEXT SECTION HELPERS
// ═══════════════════════════════════════════════════════════════════════════

/**
 * Read usage limits from cache file (written by usage-context-awareness.cjs)
 * @returns {Object|null} Usage data or null if unavailable/stale
 */
 ...
 * @returns {string|null} Formatted time (e.g., "2h 30m") or null
 */
...
/**
 * Format percentage with warning level annotation
 * @param {number} value - Percentage value
 * @param {string} label - Label prefix
 * @returns {string} Formatted string with [WARNING] or [CRITICAL] if applicable
 */
...
/**
 * Build usage section from rate limit cache
...
function buildReminder(params) {
  ...
  // Respect hooks config - skip sections when corresponding hook is disabled
...
/**
 * Build complete reminder context (unified entry point)
 *
 * @param {Object} [params]
 ...
 * @returns {{ content: string, lines: string[], sections: Object }}
 */
function buildReminderContext({ sessionId, config, staticEnv, configDirName = '.claude', baseDir } = {}) {
  ...
  const params = {
    ...
    catalogScript,
    skillsVenv,
    reportsPath: baseDir ? path.join(baseDir, planCtx.reportsPath) : planCtx.reportsPath,
    plansPath: baseDir ? path.join(baseDir, plansPathRel) : plansPathRel,
    docsPath: baseDir ? path.join(baseDir, docsPathRel) : docsPathRel,
