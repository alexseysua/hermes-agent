...
function getSessionTempPath(sessionId) {
  try {
    const { sanitizeSessionId } = require('./ck-paths.cjs');
    return path.join(os.tmpdir(), `ck-session-${sanitizeSessionId(sessionId)}.json`);
  } catch {
    // ck-paths.cjs may not exist in all contexts - fallback to basic sanitization
    const safe = String(sessionId || '').replace(/[^a-zA-Z0-9_-]/g, '-').slice(0, 64);
    return path.join(os.tmpdir(), `ck-session-${safe}.json`);
  }
}
 ...
 * @param {number} [options.timeout] - Timeout in ms (default: DEFAULT_EXEC_TIMEOUT_MS)
...
function loadConfig(options = {}) {
  ...
  try {
    ...
    // Hooks configuration (per-hook enable/disable toggles)
    result.hooks = merged.hooks || DEFAULT_CONFIG.hooks;
    // Statusline mode ('full' | 'minimal' | 'off')
...
function resolveNamingPattern(planConfig, gitBranch) {
  ...
  if (fullIssue) {
    ...
    pattern = pattern.replace(/-?\{issue\}-?/, (match) =>
...
// ============================================================================
// Memory System Utilities (Phase 3: Memory System Integration)
// ============================================================================

/**
 * Get memory paths for current project
 * Returns standardized paths for all memory scopes
 * @returns {Object} Memory path configuration
 */
function getMemoryPaths() {
  const { encodeCwdPath } = require('./ck-paths.cjs');
  const cwd = process.cwd();
  const homedir = os.homedir();

  return {
    userMemory: path.join(homedir, '.claude', 'CLAUDE.md'),
    userRules: path.join(homedir, '.claude', 'rules'),
    projectMemory: path.join(cwd, '.claude', 'CLAUDE.md'),
    projectRules: path.join(cwd, '.claude', 'rules'),
    localMemory: path.join(cwd, 'CLAUDE.local.md'),
    autoMemory: path.join(homedir, '.claude', 'projects', encodeCwdPath(cwd), 'memory')
  };
}

/**
 * Count lines in a file (for memory size reporting)
...
/**
 * Count files in a directory (for rules/memory directory reporting)
 * @param {string} dirPath - Path to directory
 * @returns {number} File count or 0 if directory doesn't exist
 */
...
/**
 * Check if memory is enabled and accessible
 * Returns detailed status for each memory scope
 * @param {Object} config - Loaded config (from loadConfig())
 * @returns {Object} Memory status with existence flags and counts
 */
function getMemoryStatus(config) {
  ...
  return {
    enabled,
    promptOnFirstUse: config.memory?.promptOnFirstUse ?? true,
    scopes: config.memory?.scopes ?? ['project', 'local'],
    maxAutoMemoryLines: config.memory?.maxAutoMemoryLines ?? 200,
    user: {
      exists: fs.existsSync(paths.userMemory),
      path: paths.userMemory,
      lines: countFileLines(paths.userMemory),
      rulesCount: countDirFiles(paths.userRules)
    },
    project: {
      exists: fs.existsSync(paths.projectMemory),
      path: paths.projectMemory,
      lines: countFileLines(paths.projectMemory),
      rulesCount: countDirFiles(paths.projectRules)
    },
    local: {
      exists: fs.existsSync(paths.localMemory),
      path: paths.localMemory,
      lines: countFileLines(paths.localMemory)
    },
    auto: {
      exists: fs.existsSync(paths.autoMemory),
      path: paths.autoMemory,
      filesCount: countDirFiles(paths.autoMemory)
    }
  };
}

/**
 * Build memory status output string (for session init)
 * @param {Object} memoryStatus - Result from getMemoryStatus()
 * @returns {string|null} Memory status string or null if disabled
 */
function buildMemoryOutput(memoryStatus) {
  if (!memoryStatus.enabled) return null;

  const activeScopes = [];

  if (memoryStatus.user.exists) {
    activeScopes.push(`user(${memoryStatus.user.lines}L)`);
  }
  if (memoryStatus.project.exists) {
    activeScopes.push(`project(${memoryStatus.project.lines}L)`);
  }
  if (memoryStatus.local.exists) {
    activeScopes.push(`local(${memoryStatus.local.lines}L)`);
  }
  if (memoryStatus.auto.exists && memoryStatus.auto.filesCount > 0) {
    activeScopes.push(`auto(${memoryStatus.auto.filesCount} files)`);
  }

  if (activeScopes.length === 0) return null;

  return `Memory: ${activeScopes.join(', ')}`;
}

// ============================================================================
// PR Context Utilities (Phase 5: Session & PR Linking)
// ============================================================================

/**
 * Check if gh CLI is available
 * Uses execFileSync for security (no shell interpolation)
 * @returns {boolean} True if gh CLI is installed and accessible
 */
function isGhCliAvailable() {
  try {
    const { execFileSync } = require('child_process');
    execFileSync('gh', ['--version'], { stdio: 'pipe', timeout: 2000 });
    return true;
  } catch (e) {
    return false;
  }
...
/**
 * Parse PR identifier from --from-pr argument
 * Supports: 123, #123, https://github.com/owner/repo/pull/123
 * @param {string} arg - PR argument
 * @returns {string|null} Normalized PR number (numeric only) or null if invalid
 */
function parsePrIdentifier(arg) {
  if (!arg) return null;

  // URL: extract PR number from GitHub pull request URL
  const urlMatch = arg.match(/github\.com\/[^/]+\/[^/]+\/pull\/(\d+)/);
  if (urlMatch) return urlMatch[1];

  // Number with optional # prefix
  const numMatch = String(arg).match(/^#?(\d+)$/);
...
/**
 * Extract PR context from GitHub using gh CLI
 * Uses execFileSync to prevent command injection
 * @param {string|number} prIdentifier - PR number (must be validated/numeric)
 * @returns {Object|null} PR context or null if unavailable
 */
function extractPrContext(prIdentifier) {
  if (!isGhCliAvailable()) return null;

  // Validate: must be numeric (parsePrIdentifier ensures this)
  const prNum = String(prIdentifier).replace(/^#/, '');
  if (!/^\d+$/.test(prNum)) return null;

  try {
    const { execFileSync } = require('child_process');
    const json = execFileSync(
      'gh',
      ['pr', 'view', prNum, '--json', 'number,title,headRefName,url,body'],
      { encoding: 'utf8', timeout: 5000, stdio: ['pipe', 'pipe', 'pipe'] }
    );
    const parsed = JSON.parse(json.trim());

    // Truncate body to prevent large PR descriptions bloating session state
    if (parsed.body && parsed.body.length > 500) {
      parsed.body = parsed.body.slice(0, 500) + '...';
    }
...
/**
 * Build PR context output string (for session init)
 * @param {Object|null} prContext - PR context from extractPrContext()
 * @returns {string|null} PR status string or null if no PR linked
 */
function buildPrOutput(prContext) {
  if (!prContext || !prContext.number) return null;
  return `PR: #${prContext.number} - ${prContext.title || 'Untitled'}`;
}
