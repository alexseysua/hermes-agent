...
async function main() {
  try {
    ...
    // Language (thinking + response, if configured) - delegate to context-builder
    const languageLines = buildLanguageSection({ thinkingLanguage, responseLanguage });
    lines.push(...languageLines);
...
main().catch(err => {
  try {
    ...
    require('fs').appendFileSync(
      path.join(logsDir, 'subagent-init-crash.log'),
