...
// Import scout-checker facade and error formatter
...
try {
  ...
  // claudeDir: the project's .claude/ directory (for .ckignore lookup)
  const claudeDir = path.join(process.cwd(), '.claude');

  // Delegate all logic to scout-checker facade
  ...
  // Format and output error message based on block type
  if (result.isBroadPattern) {
    const errorMsg = formatBroadPatternError(
      { blocked: true, reason: result.reason, suggestions: result.suggestions, pattern: result.pattern },
      claudeDir
    );
