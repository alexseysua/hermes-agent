...
try {
  ...
  /**
   * Check if we should fetch (throttled by interval)
   */
  ...
  /**
   * Write cache entry: { timestamp, status, data }
   */
  function writeCache(status, data = null) {
    fs.writeFileSync(
      USAGE_CACHE_FILE,
      JSON.stringify({ timestamp: Date.now(), status, data })
    );
  }

  /**
   * Fetch usage limits from Anthropic OAuth API and write to cache.
   * Always writes status (available|unavailable) so statusline has fallback.
   */
  ...
  /**
   * Main hook execution - fetch and cache usage limits, then continue
   */
  async function main() {
    ...
    try {
      ...
      // Detect hook type: UserPromptSubmit has a 'prompt' string field
