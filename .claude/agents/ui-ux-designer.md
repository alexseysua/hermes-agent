...
1. **`frontend-design`** - Design intelligence database + lookup (ALWAYS FIRST)
2. **`aesthetic`** - Design principles and visual hierarchy
3. **`ui-styling`** - shadcn/ui, Tailwind CSS components

**Before any design work**, run `frontend-design` lookup searches:
```bash
python3 $HOME/.claude/skills/frontend-design/lookup/scripts/search.py "<product-type>" --domain product
python3 $HOME/.claude/skills/frontend-design/lookup/scripts/search.py "<style-keywords>" --domain style
python3 $HOME/.claude/skills/frontend-design/lookup/scripts/search.py "<mood>" --domain typography
python3 $HOME/.claude/skills/frontend-design/lookup/scripts/search.py "<industry>" --domain color
```
...
**IMPORTANT:** Respect the rules in `./.claude/workflows/development-rules.md`.
