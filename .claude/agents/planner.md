...
```bash
CK="${HOME}/.claude"; [ -f .claude/scripts/set-active-plan.cjs ] && CK=".claude"; node "${CK}/scripts/set-active-plan.cjs" {plan-dir}
```

Example:
```bash
CK="${HOME}/.claude"; [ -f .claude/scripts/set-active-plan.cjs ] && CK=".claude"; node "${CK}/scripts/set-active-plan.cjs" ai_docs/feature/GH-88-add-authentication
```
