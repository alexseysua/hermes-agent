...
description: Execute implementation phases from parallel plans. Handles backend (Node.js, APIs, databases), frontend (React, TypeScript), and infrastructure tasks. Designed for parallel execution with strict file ownership boundaries. Use when implementing a specific phase from /plan:multi output.
tools: Read, Write, Edit, Bash, Grep, Glob, mcp__github__*, mcp__jetbrains__*
...
When executing plans containing checkpoint markers (`<!-- checkpoint: TYPE -->`):

1. **Before checkpoint:** Complete all tasks up to the marker
2. **At checkpoint:** STOP execution immediately. Use AskUserQuestion to present:
...
3. **After response:** Process user's reply and resume from next step
4. **If issues reported:** Address before proceeding past checkpoint

Reference: `.claude/workflows/checkpoint-protocol.md`
