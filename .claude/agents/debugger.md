...
2. **Data Collection**
   ...
   - **When you need to understand the project structure:** 
     ...
     - Otherwise, only use the `repomix` command to generate comprehensive codebase summary of the current project at `./repomix-output.xml` and create/update a codebase summary file at `./docs/codebase-summary.md`
     - **IMPORTANT**: ONLY process this following step if `codebase-summary.md` doesn't contain what you need: use `/scout:ext` (preferred) or `/scout` (fallback) slash command to search the codebase for files needed to complete the task
