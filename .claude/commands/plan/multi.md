---
description: ⚡⚡⚡ Multi — create detailed plan with parallel-executable phases
argument-hint: [task]
---
...
| `auto` | Automatically run validation interview inline |
| `off` | Skip validation step entirely |

**If mode is `prompt`:** Use `AskUserQuestion` tool with options above.
**If user chooses validation or mode is `auto`:** Run validation inline (same as `/plan:deep` validation step).
