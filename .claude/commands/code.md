...
argument-hint: [plan] [--auto] [--skip-test]
---
...
Parse `$ARGUMENTS` for flags (strip flags before passing plan path to core logic):
- `--auto` → skip user approval gate (Step 5 becomes automatic, no blocking wait)
- `--skip-test` → skip testing phase (Step 3 is skipped entirely)
...
**If `--skip-test` flag detected:** Skip this step entirely — mark complete and proceed to Step 4.
...
**If `--auto` flag detected:** Skip approval gate, auto-approve, proceed to Step 6.
