...
When users ask "what command should I use?" or seem unfamiliar with ClaudeKit, show this first:

```
What do you want to do?
├── Fix a bug              → /fix [description]
│   └── Complex/multi-file?  → /fix:deep  or  /fix:multi
├── Build a feature        → /cook [description]
│   └── Parallel phases?    → /cook:multi
├── Plan first             → /plan [task]
│   ├── Compare approaches? → /plan:two
│   ├── Deep research?      → /plan:deep
│   └── Parallel phases?    → /plan:multi
├── Execute a plan         → /code [plan-path]
│   └── Parallel?           → /code:multi
├── Design UI              → /design [task]
│   ├── Research-backed?    → /design:immersive
│   └── Three.js?           → /design:3d
├── Write content          → /content [brief]
│   └── Conversion optim.?  → /content:cro
├── Bootstrap project      → /bootstrap [description]
├── Explore codebase       → /scout [query]
├── Run tests              → /test
├── Review code            → /review:codebase
└── Get help               → /ck-help [topic]
```
