...
argument-hint: [user-requirements] [--auto]
---
...
Parse `$ARGUMENTS` for flags (strip before passing requirements to core logic):
- `--auto` → skip all approval/confirmation gates; proceed automatically through all phases without user interaction checkpoints
...
* If `--auto` flag detected: skip clarification questions, proceed directly to Research using provided requirements as-is.
* Otherwise: use `AskUserQuestion` to ask clarifying questions (1 at a time, wait for answer).
...
3. If `--auto` flag: auto-select recommended tech stack. Otherwise: ask user to review and approve; repeat until approved.
...
* If `--auto` flag: auto-approve plan and proceed to Wireframe & Design. Otherwise: ask user to review and approve; repeat if changes requested.
...
* If `--auto` flag: auto-proceed with generated design guidelines. Otherwise: ask user to review and approve; repeat if changes requested.
