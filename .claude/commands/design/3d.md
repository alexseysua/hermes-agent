...
1. **`frontend-design`** - Design intelligence database + lookup (ALWAYS ACTIVATE FIRST)
2. **`threejs`** - Three.js/WebGL expertise

**Ensure token efficiency while maintaining high quality.**

## Workflow:
0. **FIRST**: Run `frontend-design` lookup searches for 3D design context:
   ```bash
   python3 $HOME/.claude/skills/frontend-design/lookup/scripts/search.py "<product-type>" --domain product
   python3 $HOME/.claude/skills/frontend-design/lookup/scripts/search.py "immersive 3d" --domain style
   python3 $HOME/.claude/skills/frontend-design/lookup/scripts/search.py "animation" --domain ux
   ```
...
4. Leverage all available Gemini skills (ai-multimodal) for asset generation and validation.
