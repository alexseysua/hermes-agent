...
CATEGORY_GUIDES = {
    ...
    "plan": {
        ...
        "workflow": [
            ...
            ("Deep research", "`/plan:deep` \"complex task\""),
            ("Parallel plan", "`/plan:multi` \"large feature\""),
            ("Execute plan", "`/code` (runs the plan)"),
        ],
        "tip": "/plan:deep includes automatic validation interview before implementation",
    },
    "cook": {
        ...
        "workflow": [
            ...
            ("Parallel", "`/cook:multi` \"large feature\""),
            ("Test", "`/test`"),
        ],
    ...
    "bootstrap": {
        "title": "Project Setup",
        "workflow": [
            ("Quick start", "`/bootstrap --auto` \"requirements\""),
            ("Full setup", "`/bootstrap` \"detailed requirements\""),
            ("Parallel", "`/bootstrap:multi` \"large project\""),
        ],
    ...
    "test": {
        ...
        "workflow": [
            ("Run tests", "`/test`"),
            ("Fix failures", "`/fix` \"test failures\""),
        ],
    ...
    "design": {
        "title": "Design",
        "workflow": [
            ("Any design", "`/design` \"description or path\""),
            ("Immersive", "`/design:immersive` \"description\""),
    ...
    "content": {
        ...
        "workflow": [
            ("Any content", "`/content` \"requirements\""),
            ("Optimize", "`/content:cro`"),
        ],
        "tip": "/content auto-detects: quick write, research-backed, or enhance existing",
    },
    "integrate": {
        "title": "Integration",
        "workflow": [
...
def show_overview(data: dict, prefix: str) -> None:
    """Display overview with decision tree and quick start guide."""
    ...
    print()
    print("## Quick Start Decision Tree")
    print()
    print("**What do you want to do?**")
    print(f"```")
    print(f"├── Fix a bug              → /{p}fix [description]")
    print(f"│   └── Complex/multi-file?  → /{p}fix:deep  or  /{p}fix:multi")
    print(f"├── Build a feature        → /{p}cook [description]")
    print(f"│   └── Parallel phases?    → /{p}cook:multi")
    print(f"├── Plan first             → /{p}plan [task]")
    print(f"│   ├── Compare approaches? → /{p}plan:two")
    print(f"│   ├── Deep research?      → /{p}plan:deep")
    print(f"│   └── Parallel phases?    → /{p}plan:multi")
    print(f"├── Execute a plan         → /{p}code [plan-path]")
    print(f"│   └── Parallel?           → /{p}code:multi")
    print(f"├── Design UI              → /{p}design [task]")
    print(f"│   ├── Research-backed?    → /{p}design:immersive")
    print(f"│   └── Three.js?           → /{p}design:3d")
    print(f"├── Write content          → /{p}content [brief]")
    print(f"│   └── Conversion optim.?  → /{p}content:cro")
    print(f"├── Bootstrap project      → /{p}bootstrap [description]")
    print(f"├── Explore codebase       → /{p}scout [query]")
    print(f"├── Run tests              → /{p}test")
    print(f"├── Review code            → /{p}review:codebase")
    print(f"└── Get help               → /{p}ck-help [topic]")
    print(f"```")
    print()
    print(f"*Tip: `/{p}cook` plans internally — use `/{p}plan` → `/{p}code` only when you want an explicit plan file.*")
    print()
...
def show_config_guide() -> None:
    ...
    print("- `mode: \"off\"` - Skip validation entirely")
    print()
