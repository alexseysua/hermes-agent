 ...
 *
 * Options:
 *   --prefix <type>    Branch prefix (feat|fix|refactor|docs|test|chore|ci|perf)
...
const ALLOWED_PREFIXES = ['feat', 'fix', 'chore', 'refactor', 'docs', 'test', 'ci', 'perf'];
if (!ALLOWED_PREFIXES.includes(branchPrefix)) {
  branchPrefix = 'feat'; // fallback to safe default
}
