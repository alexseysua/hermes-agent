...
A BM25-powered search engine for UI patterns, design styles, typography, and product recommendations is available in `lookup/`.

**Usage:**
```bash
python3 $HOME/.claude/skills/frontend-design/lookup/scripts/search.py "glassmorphism saas dashboard"
python3 $HOME/.claude/skills/frontend-design/lookup/scripts/search.py "react hooks best practices"
python3 $HOME/.claude/skills/frontend-design/lookup/scripts/search.py "luxury font pairing"
```

**Data available** (`lookup/data/`):
- `styles.csv` — 58 visual style categories (Glassmorphism, Brutalism, Aurora UI, etc.)
- `typography.csv` — 57 font pairings with Google Fonts import snippets
- `products.csv` — 96 product type → style recommendations
- `ux-guidelines.csv` — 99 UX issues with do/don't code examples
- `prompts.csv` — AI prompt templates and CSS checklists per style
- `charts.csv` — Chart type selection guide
- `colors.csv` — Color palette reference
- `landing.csv` — Landing page patterns
- `stacks/` — Framework-specific guidelines (react, nextjs, vue, svelte, swiftui, react-native, flutter, html-tailwind)

Use `search.py` to surface relevant patterns before designing. Pass 2-5 keywords describing the product type, aesthetic, or technology stack.