...
4. **Update on create**: Run `CK="${HOME}/.claude"; [ -f .claude/scripts/set-active-plan.cjs ] && CK=".claude"; node "${CK}/scripts/set-active-plan.cjs" {plan-dir}`
...
```
research → planning → [if stuck] problem-solving → implementation → debugging
```

This skill is the **second step** in the workflow. Consumes research reports to produce implementation plans. Hand off plans to implementation agents (fullstack-developer, etc.).
